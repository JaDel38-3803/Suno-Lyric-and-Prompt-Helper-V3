...
## Producing a Fully Instrumental (Vocal-Free) Track

There is no dedicated `[no vocals]` meta tag, and writing "no vocals" as prose in the Style box is unreliable — negative instructions written as plain text tend to be ignored there, the same failure mode covered in the Exclude Styles Anti-Prompt section above. Do not rely on Style-box prose to suppress vocals.

The dependable combination:

1. Toggle the platform's Instrumental option on, where available
2. Add `[Instrumental]` in the Lyrics box (or `[Instrumental Break]` for a vocal-free section inside an otherwise-sung song)
3. Put `vocals` in the Exclude Styles box

For a vocal-free *section* inside a song that has vocals elsewhere, use `[Instrumental Break]` at that point in the Lyrics box rather than suppressing vocals for the whole track.
