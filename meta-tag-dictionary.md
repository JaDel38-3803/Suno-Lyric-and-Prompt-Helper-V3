...
### `[decrescendo]`
What it does: Gradual decrease in intensity — a named alternate to `[diminuendo]`. Same function; use whichever reads more naturally for the section.
Common values: `gradual`, `sudden`, `subtle`
Example: `[decrescendo: Strings thin out into the bridge]`
...
## Instrument-Specific Solo Tags

These target a solo section to a named instrument rather than the generic `[Solo]` tag documented in `suno-tag-mechanics.md`. Use the specific form whenever the instrument matters to the arrangement — Suno responds more reliably to a named instrument than to a bare `[Solo]` when the genre supports multiple lead options.

### `[Guitar Solo]`
What it does: Guitar-focused instrumental passage.
Example: `[Guitar Solo]` on its own line between the bridge and final chorus.

### `[Piano Solo]`
What it does: Piano-focused instrumental passage.
Example: `[Piano Solo]` for a stripped-back midsong break.

### `[Drum Solo]`
What it does: Percussion-focused passage.
Example: `[Drum Solo]` heading into a breakdown.

### `[Bass Solo]`
What it does: Bass-focused passage.
Example: `[Bass Solo]` in a funk or groove-metal breakdown.

### `[Saxophone Solo]`
What it does: Saxophone-focused passage.
Example: `[Saxophone Solo]` in a jazz or blues-rock bridge.

### `[Synth Solo]`
What it does: Synthesizer lead passage.
Example: `[Synth Solo]` in an EDM or synthwave build.

### `[Instrumental Intro]`
What it does: An instrumental opening section, distinct from a generic `[Intro]` — signals no vocals should enter until the first tagged verse.
Example: `[Instrumental Intro]` as the first line, before `[Verse 1]`.

### `[Strings Rise]`
What it does: A string-section swell, typically used to build into a chorus or climax.
Example: `[Strings Rise]` immediately before `[Final Chorus]`.

### `[Percussion Break]`
What it does: A rhythm-focused breakdown foregrounding percussion over melodic elements.
Example: `[Percussion Break]` between the bridge and the final chorus.
...
What it does: Switches the lead vocal identity for the tagged section — useful mid-song for duets or a perspective shift without a full `[duet]` structure.
Example: `[Female Vocal]` on a verse that shifts to a second character's point of view.

### `[Rap]`
What it does: Rap delivery for the tagged section.
Example: `[Rap]` on a verse inside an otherwise sung genre, for a rap-rock or nu-metal hybrid.

### `[Spoken Word]`
What it does: Spoken, not sung, delivery.
Example: `[Spoken Word]` for a narrated intro or interlude.

### `[Scream]`
What it does: Screamed or shouted delivery — metal, punk, hardcore.
Example: `[Scream]` on a breakdown's final line.

### `[Humming]`
What it does: Hummed melody, no lyrics.
Example: `[Humming]` for a wordless intro or outro reprise of the hook melody.

### `[Backing Vocals]`
What it does: Background vocal parts distinct from the lead — a plainer alternative to `[choir]` when the effect wanted is a few supporting voices rather than a full choral sound.
Example: `[Backing Vocals]` under a chorus's second half.
